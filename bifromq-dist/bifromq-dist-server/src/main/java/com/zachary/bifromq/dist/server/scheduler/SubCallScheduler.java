...
import static com.zachary.bifromq.sysprops.parser.BifroMQSysProp.DIST_MAX_UPDATES_IN_BATCH;
        ...
        private class BatchSubRequest implements IBatchCall<SubCall, SubCallResult> {
            ...
            public CompletableFuture<Void> execute() {
                ...
                if (!addTopicFilter.isEmpty()) {
                    reqBuilder.setAddTopicFilter(com.zachary.bifromq.dist.rpc.proto.AddTopicFilter.newBuilder()
                ...
                if (!remTopicFilter.isEmpty()) {
                    reqBuilder.setRemoveTopicFilter(com.zachary.bifromq.dist.rpc.proto.RemoveTopicFilter.newBuilder()
                ...
                if (!insertMatchRecord.isEmpty()) {
                    reqBuilder.setInsertMatchRecord(com.zachary.bifromq.dist.rpc.proto.InsertMatchRecord.newBuilder()
                        .putAllRecord(insertMatchRecord)
                        .build());
                }
                if (!joinMatchGroup.isEmpty()) {
                    reqBuilder.setJoinMatchGroup(com.zachary.bifromq.dist.rpc.proto.JoinMatchGroup.newBuilder()
                ...
                if (!delMatchRecord.isEmpty()) {
                    reqBuilder.setDeleteMatchRecord(com.zachary.bifromq.dist.rpc.proto.DeleteMatchRecord.newBuilder()
