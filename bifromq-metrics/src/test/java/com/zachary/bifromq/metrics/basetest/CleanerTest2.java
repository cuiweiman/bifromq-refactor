...
public class CleanerTest2 {
    ...
    /**
     * Animal 作为 资源, 内置了 自动资源回收器 try(Resource) catch() 可以触发自动垃圾回收
     */
    static class Animal implements Runnable, AutoCloseable {
        ...
        public Animal(String name) {
            this.name = name;
            System.out.printf("【构造】%s 出生 %n", name);
            Cleaner CLEANER = Cleaner.create();
            this.cleanable = CLEANER.register(this, this);
        }

        public void logical() {
            System.out.println("执行业务逻辑方法 " + this.name);
            try {
        ...
        public void run() {
            // 资源回收 业务
            System.out.printf("【回收】 %s动物死亡, 资源回收%n", this.name);
        }
    ...
    public static void main(String[] args) {
        try (Animal animal = new Animal("大黄")) {
            //中间可以执行一些相关的代码
            animal.logical();
